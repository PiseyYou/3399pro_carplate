    ...
    )
{
    ...
    for (auto it=this->veRect_aftboxes.begin();it!=this->veRect_aftboxes.end();it++)
    {
         ...
         if ((*it)[4]<=-5) 
         {
             ...
             if (this->nAp_max >=2 && (*it)[5] != -1 && this->stPlateMax_res != this->stLast_res)
             {
                 stFinal_result = (stPlateMax_res!="")? this->stType_max_type+"-"+this->stPlateMax_res:"Null"; // get the final result
         ...
         if (nTrack >=5 && vehicle_box.x < matImg_src.cols/2 && vehicle_box.y+vehicle_box.height > matImg_src.rows/3 &&
             nRecog_time != -1 && nMiss_time ==0)
         {
            ...
            if (stResult_plate.size()==1)
            {
               ...
               if ((nRecog_time == 100 || this->nAp_max >=7 || vehicle_box.y+vehicle_box.height > 4*matImg_src.rows/5)&&
                   this->stPlateMax_res != this->stLast_res)
               {
                  //printf("xxxxxx!!!\n");
     ...
     )
{
     ...
     if (Plateboxes.size()==0)stResult_plate.push_back(std::pair<std::string,std::string>(stC_result,this->stPlate_type));
...
std::string trackSystem_pro::track_pro(
     cv::Mat& matImg_src 
)
{
     //usleep(30000);
     //printf("123123\n");
     std::vector<cv::Rect> Vehicleboxes;
     Vehicleboxes = RknnDetInference_Process(matImg_src, Vehicle_detModel, 100, 100, 3, vehcile_boxPriors, VEHICLE_NUM_RESULTS); //detect vehicle
     cal_rectpro(Vehicleboxes,matImg_src);

     std::string result = output_platepro(matImg_src);
     //std::string result = "Null";
     //if (result!="Null")std::cout<< "the final result is " << result << std::endl;
     /*
