...
int loadCoderOptions(string locationFilename, float (*boxPriors)[NUM_RESULTS])
{
    ...
    while(getline(fin, line))
    {
        ...
        while (p) {
            ...
            boxPriors[lineNum][priorIndex++] = number;  //cy cx h w
            p=strtok(nullptr, d);
        }
...
int loadCoderOptions_my(string locationFilename, float (*boxPriors)[NUM_RESULTS])
{
    ifstream fin(locationFilename);
    string line;
    int lineNum = 0;
    ...
    while(getline(fin, line))
    {
        ...
        if (lineNum == 3)
        {
           boxPriors[0][priorIndex] = (temp[0] + temp[2]) / 2; //cx
           boxPriors[1][priorIndex] = (temp[1] + temp[3]) / 2; //cy
           boxPriors[2][priorIndex] = temp[2] - temp[0];       //w
           boxPriors[3][priorIndex] = temp[3] - temp[1];       //h
...
void decodeCenterSizeBoxes(float* predictions, float (*boxPriors)[NUM_RESULTS]) {
    ...
    for (int i = 0; i < NUM_RESULTS; ++i) {
        float xcenter = predictions[i*4+0] / Y_SCALE * boxPriors[2][i] + boxPriors[0][i];
        float ycenter = predictions[i*4+1] / X_SCALE * boxPriors[3][i] + boxPriors[1][i];
        float w = (float) exp(predictions[i*4 + 2] / H_SCALE) * boxPriors[2][i];
        float h = (float) exp(predictions[i*4 + 3] / W_SCALE) * boxPriors[3][i];
...
{
    const char *img_path = "./tmp/license_plate/20191225_87.jpg";
    const char *model_path = "./tmp/license_plate/platechar_DetRkmodel_qu0106.rknn";
    const char *label_path = "./tmp/license_plate/coco_labels_list.txt";
    const char *box_priors_path = "./tmp/license_plate/Platechar_DetBox0106.txt";
    ...
    if(outputs[0].size == outputs_attr[0].n_elems*sizeof(float) && outputs[1].size == outputs_attr[1].n_elems*sizeof(float))
    {
        ...
        loadCoderOptions_my(box_priors_path, boxPriors);
        ...
        if (validCount < 100) {
            ...
            for (int i = 0; i < validCount; ++i) {
                ...
                int x1 = static_cast<int>(predictions[n * 4 + 0] * rgba.cols);
                int y1 = static_cast<int>(predictions[n * 4 + 1] * rgba.rows);
                int x2 = static_cast<int>(predictions[n * 4 + 2] * rgba.cols);
                int y2 = static_cast<int>(predictions[n * 4 + 3] * rgba.rows);
