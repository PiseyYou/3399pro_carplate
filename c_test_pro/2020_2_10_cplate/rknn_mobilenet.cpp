...
{
    const char *img_path = "./tmp/license_plate/20170502120723_鄂_0.jpg";
    const char *model_path = "./tmp/license_plate/charrecog0102.rknn";
    const char *lable_path = "./tmp/license_plate/labels.txt";
    const int output_elems = 72;

    const int img_width = 24;
    ...
    //BGR->RGB
    if (img_channels == 3)cv::cvtColor(img, img, cv::COLOR_BGR2RGB);
    else cv::cvtColor(img, img, cv::COLOR_BGR2GRAY);
    // Load model
