    ...
    for (auto it=this->veRect_aftboxes.begin();it!=this->veRect_aftboxes.end();it++)
    {
         ...
         if ((*it)[4]<=-5) 
         {
             ...
             if (this->nAp_max >=2 && (*it)[5] >= 2 && this->stPlateMax_res != this->stLast_res)
             {
                 ...
                 stFinal_result = (stPlateMax_res!="")? this->stType_max_type+"-"+this->stPlateMax_res + rect_res:"Null"; // get the final result
             ...
             nRecog_time != -1 && nMiss_time ==0)
         {
            ...
            if (stResult_plate.size()==1)
            {
               ...
               if ((nRecog_time == 100 /*!!!*/|| this->nAp_max >=5 || vehicle_box.y+vehicle_box.height > 4*matImg_src.rows/5)&&
