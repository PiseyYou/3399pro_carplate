...
static int createSocket(int type) {
  ...
  //packet_buffer = (uint8_t*)malloc(rtp_size);
  if(init_mpp == 0)
  {
	mppDecoder();
	init_mpp = 1;
...
int readSocket(UsageEnvironment& env,
  ...
  if (bytesRead > 0) 
  {
        ...
        // 复制到缓冲区
        memcpy(rtp_content+rtp_write, buffer, bytesRead);
        rtp_write += bytesRead;
   }
   //printf("rtp_read = %d,rtp_write = %d\n",rtp_read,rtp_write);
    int hasNext = 0;
